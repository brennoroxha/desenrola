-- Rodar no SQL Editor do Supabase (o mesmo projeto usado pelo Roleta 2026).
-- Todas as tabelas ficam fechadas por RLS; apenas service_role escreve/lê.
...
-- 2. Sessões de chat / código de acordo
...
-- Colunas de notificação (para instalações antigas)
alter table public.desenrola_pix_transactions add column if not exists notified_gerado boolean not null default false;
alter table public.desenrola_pix_transactions add column if not exists notified_aprovado boolean not null default false;
alter table public.desenrola_pix_transactions add column if not exists gateway text not null default 'freepay';
create index if not exists desenrola_pix_transactions_gateway_idx on public.desenrola_pix_transactions(gateway);

-- 5b. Configurações gerais (key/value). Ex.: gateway ativo (freepay | blackcat).
...
-- Seed opcional de credenciais Blackcat (rodar manualmente):
-- insert into public.desenrola_api_credentials (provider, secret_key)
-- values ('blackcat', 'SUA_API_KEY_BLACKCAT')
-- on conflict (provider) do update set secret_key = excluded.secret_key, atualizado_em = now();

-- 4. Eventos crus do webhook
...
-- 5. Credenciais de APIs externas.
...
-- 6. Eventos de página (funil / passos do visitante)
...
-- 7. Comprovantes enviados pelo cliente (armazenados como base64 em jsonb).
...
-- 8. Pixels do Google Ads (múltiplos, gerenciados pelo painel admin).
