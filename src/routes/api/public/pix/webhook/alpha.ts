...
export const Route = createFileRoute("/api/public/pix/webhook/alpha")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const { id, status: statusRaw, paidAt } = parseWebhookPayload(payload, "alpha");
        const gateway = "alpha";

        console.log("[pix webhook alpha]", JSON.stringify({ id, status: statusRaw, paidAt }));
        ...
        try {
          ...
          if (id) {
            ...
            if (upErr) console.error("[pix webhook alpha] update error", upErr);
          ...
          console.error("[pix webhook alpha] supabase failed", err);
        }
