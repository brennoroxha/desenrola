  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        console.log("[pix webhook]", JSON.stringify({ gateway, id, status: statusRaw, paidAt }));
        ...
        try {
          ...
          if (id) {
            ...
            if (upErr) console.error("[pix webhook] update error", upErr);
          ...
          console.error("[pix webhook] supabase failed", err);
        }
